...
export class ModelsComponent implements OnInit {
  ...
  constructor(private usersService:WebmasterPanelService,private route:Router) { 
    this.update = new FormGroup({
      model_car_name: new FormControl(''),
 
    });
    this.addmodel = new FormGroup({
      model_car_name: new FormControl('', [Validators.required]),
 
    });
  }

  ngOnInit(): void {
    this.usersService.getallmodels().subscribe(data=>{
      // debugger
...
addnewmodel (f:any){
  const formData = new FormData();
  formData.append('model_car_name', this.addmodel.value.model_car_name);
  let data=f.value
